...
func GetTodo(db *sqlx.DB, id int) (models.Todo, error) {
	...
	query := "SELECT id, title, content, created_at, updated_at FROM todos WHERE id = ?"
...
func ListTodos(db *sqlx.DB) ([]models.Todo, error) {
	dbTodos := []struct {
		ID        int       `db:"id"`
		Title     string    `db:"title"`
		Content   string    `db:"content"`
...
func CreateTodo(db *sqlx.DB, todo models.Todo) (models.Todo, error) {
	...
	query := "INSERT INTO todos (title, content, created_at, updated_at) VALUES (:title, :content, :created_at, :updated_at)"
...
func UpdateTodo(db *sqlx.DB, todo models.Todo) (models.Todo, error) {
	...
	query := "UPDATE todos SET title = :title, content = :content, updated_at = :updated_at WHERE id = :id"
